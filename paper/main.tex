...
\begin{theorem}[Relational Multiplicity--Index Transfer]
If \(\Gamma_A\) is an isomorphism, then
\[
\boxed{\operatorname{ord}_{z=0}\det A(z)=\dim\ker A(0).}
\]
\end{theorem}
...
\]
Since \(b(z)d(z)^{-1}c(z)=O(z^2)\) and \(a(z)=z\Gamma_A+O(z^2)\),
\[
\det A(z)=z^r u(z),\qquad u(0)\ne0,
\]
which proves the claim.
\end{proof}

This theorem is independent of BSD. It gives a precise mechanism by which determinant multiplicity equals the number of zero modes under a transverse crossing.
